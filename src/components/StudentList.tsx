...
export const StudentList: React.FC<StudentListProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Search & Filter Bar */}
      <div className="bg-[#F7F5EE] p-4 rounded-xl border border-[#E6E2D3] shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-[#FDFCF8] border border-[#DDD8C5] rounded-lg text-sm text-[#4A453E] placeholder-[#9E988D] focus:outline-none focus:ring-2 focus:ring-[#5A5A40] focus:bg-white transition"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[#8C857B] hover:text-[#4A453E]"
              >
                Clear
              </button>
            )}
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-[#4A453E] bg-[#EFECE1] hover:bg-[#E6E2D3] rounded-lg transition border border-[#DDD8C5]"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-white bg-[#5A5A40] hover:bg-[#484833] rounded-lg shadow-sm transition border border-[#484833]"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-[#E6E2D3] text-xs">
          ...
          <div>
            <label className="block font-medium text-[#787267] mb-1">Fee Payment Status</label>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="w-full bg-[#FDFCF8] border border-[#DDD8C5] rounded-lg px-2.5 py-1.5 text-[#4A453E] focus:outline-none focus:ring-2 focus:ring-[#5A5A40]"
            >
          ...
          {/* Category Filter */}
          <div>
            <label className="block font-medium text-[#787267] mb-1">Caste Category</label>
            <select
              value={casteFilter}
          ...
          {/* Exam Type Filter */}
          <div>
            <label className="block font-medium text-[#787267] mb-1">Exam Type</label>
            <select
          ...
          {/* Class / Stream Filter */}
          <div>
            <label className="block font-medium text-[#787267] mb-1">Class / Stream</label>
            <select
              value={streamFilter}
      ...
      {/* Stats Breakdown Bar */}
      <div className="bg-[#4A453E] text-white rounded-xl p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 border border-[#3E3A33]">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full md:w-auto text-xs">
          <div>
            <p className="text-[#DDD8C5]">Total Filtered</p>
            <p className="text-base font-bold text-white">{stats.totalCount} Students</p>
          </div>
          <div>
            <p className="text-[#DDD8C5]">Total Fee Target</p>
            <p className="text-base font-bold text-[#FDFCF8]">₹{stats.totalFeeExpected.toLocaleString('en-IN')}</p>
          </div>
          <div>
            <p className="text-[#DDD8C5]">Collected Revenue</p>
            <p className="text-base font-bold text-[#A3C9A8]">₹{stats.totalFeeCollected.toLocaleString('en-IN')}</p>
          </div>
          <div>
            <p className="text-[#DDD8C5]">Total Online Charges (+₹30)</p>
            <p className="text-base font-bold text-[#E6E2D3]">₹{stats.totalOnlineChargesExpected.toLocaleString('en-IN')}</p>
          </div>
        </div>

        {stats.totalFeeDue > 0 && (
          <div className="bg-[#8C5A2B]/30 border border-[#E8D0B8]/40 rounded-lg px-3 py-2 text-xs text-[#FAF0E6] flex items-center gap-2 w-full md:w-auto justify-center">
            <AlertCircle className="w-4 h-4 text-[#E8D0B8] shrink-0" />
            <span>
              Pending Due Amount to collect: <strong>₹{stats.totalFeeDue.toLocaleString('en-IN')}</strong> ({stats.unpaidCount + stats.partialCount} students)
            </span>
          </div>
        )}
      </div>
